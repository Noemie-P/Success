...
export default {
  ...
  methods: {
    async registerUser() {
      ...
      const requestOpt = {
            method: "POST",
            body: JSON.stringify({"user_name":this.register.user_name, "type":this.register.type, "mail":this.register.mail, "password": this.register.password, "groupe":this.groupe2}),
        ...
        .then((result) => {
          ...
          //if (token) {
            ...
            this.$router.push({name:"register2FA", params: {user: jdata.data, url: jdata.url, secret: jdata.secret, qr: jdata.user}});
            //this.$router.push("/");
